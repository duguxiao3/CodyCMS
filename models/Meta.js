...
Meta.Messages.addStdMessages = function() {
  ...
  this.addMessage("cn", Meta.Messages.kOK, "成功");
  this.addMessage("cn", Meta.Messages.kTooSmall, "太小");
  this.addMessage("cn", Meta.Messages.kTooBig, "太大");
  this.addMessage("cn", Meta.Messages.kRequired, "必填");
  this.addMessage("cn", Meta.Messages.kNoNumber, "无数据");
  this.addMessage("cn", Meta.Messages.kTooLong, "太长");
  this.addMessage("cn", Meta.Messages.kTooShort, "太短");
  this.addMessage("cn", Meta.Messages.kInvalidEmail, "无效的email地址");
  this.addMessage("cn", Meta.Messages.kInvalidPhone, "无效的手机号码");
};
...
Meta.prototype.errors = function(lang) {
  if (typeof lang === "undefined") { lang = "cn"; }
  var errs = [];
  for (var iO in this.objects) {
    var O = this.objects[iO];
...
function validLang(lang) {
  return (typeof lang === "undefined") ? "cn" : lang;
}
...
function demo() {
  ...
  X.add({
    ...
    labels: {"en": "email address", "cn": "电子邮件"}
  });

  var choices = {};
  choices["cn"] = { 'M': '男性', 'F': '女性'};
  ...
  X.add({name: "phony",
    ...
    generator: Meta.Generator.textinput,
    labels: {"en": "Phone", "cn": "电话"}
  });

  var food = {};
  food["cn"] = { 'O': '早餐', 'L': '午餐', 'D': '晚餐'};
  ...
  console.log("HTML empty = "); console.log(X.html("cn"));

  X.reset();
  X.readValuesFrom(P, true);
  console.log("Params = ");console.log(P);
  console.log("Values = ");console.log(X.values());
  console.log("Errors = ");console.log(X.errors("cn"));
  console.log("CN - HTML with values = ");console.log(X.html("cn"));
